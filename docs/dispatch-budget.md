...
* **Unreadable Metrics:** In case of unreadable metrics, the Async Processor cannot take informed decisions, and therefore will assume a **dispatch budget of 0** (configurable via the `fallback` gate param). Because that is indistinguishable from a genuine reading of 0, the gate also exports `async_gate_metric_source_available` — 1 when the last evaluation read a real value, 0 when it fell back. Treat a sustained 0 as an alertable condition: the gate is not measuring anything.
